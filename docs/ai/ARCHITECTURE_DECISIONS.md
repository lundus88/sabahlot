...
**Status:** Accepted and implemented (sprint `production-read-gate-phase1`, 2026-08-04). `npx tsc --noEmit` clean, `npx eslint .` 0 errors (31 pre-existing warnings, unchanged), `npm run build` succeeds, all 17 pre-existing `.qa.ts`/`.qa.tsx` suites re-run fresh with zero regressions, `feature-gate.qa.ts` 15/15 assertions PASS (9 pre-existing + 6 new). **Not yet committed/pushed** — awaiting explicit owner approval per this sprint's brief. Production read remains inert (`PRODUCTION_READ_ENABLED_CONSTANT = false`) regardless of when this merges; activating it is a separate future decision, not authorized by this ADR.
