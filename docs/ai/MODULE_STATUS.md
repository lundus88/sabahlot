...
| Handheld GPS | Done — accuracy-grade recalibrated 2026-07-23 (A/B/C/D now 0.5/1/3m, was 5/10/25m) plus QA/QC-scope and offline-capability disclaimer copy added. CSV/KML/DXF target-file import shipped and QA-covered (`field-target-import.qa.ts`), cherry-picked from `gps-import-field-targets-kml-dxf-csv` — import-only scope, as three separate local commits on `main` (not yet pushed/merged); the rest of that branch (AR-stakeout rewrite, layout/manifest changes, GeoJSON export, active-target persistence) was deliberately left out as out-of-scope for this sprint | Alpha/Beta | No | Active | Physical-device verification of new accuracy thresholds and the import feature; owner decision on whether to separately scope the left-out AR-stakeout/GeoJSON-export/active-target-persistence portions of `gps-import-field-targets-kml-dxf-csv` |
