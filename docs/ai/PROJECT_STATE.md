...
| Dimension | Status |
|---|---|
| Cloud writes | `land_records` parent create/update and the boundary **geometry** sync are merged and wired to the local-first save flow in Dev (PR #24). Points create-only persistence is merged but not wired to the UI. **Parties** backend create+update (minus `id_number`, ADR-014) is implemented and QA-passed, and **create-only UI wiring** (four `PdfIdentityFields` entries, via new `parties-ui-sync.ts`) is now also wired into the Dev save flow — all on the pending `sprint-parties-cloud-write` branch (based on `main@9abe090`, 2026-07-25), not yet committed/pushed/merged. |
...
- **Parties cloud create+update (backend)** — `src/lib/land-records/parties-repository.ts` / `parties-write-coordinator.ts` / `parties-validation.ts` / `parties-cache.ts` (new). Implemented and QA-passed (`parties-write.qa.ts`, 30/30) on branch `sprint-parties-cloud-write` (based on `main@9abe090`, 2026-07-25) — **not yet committed/pushed/merged**, pending owner review. `id_number` (ADR-014) has no field in the writable-input type at all, so it structurally cannot reach any outbound Supabase payload; explicitly regression-tested. Full create+update exists in the backend, but only create is actually called from the UI so far — see the Active-in-Dev entry below. `index.ts` does not yet barrel-export this module (out of this sprint's Allowed Files — a future Foundation/Integration sprint should add that additively).
...
- **Parties cloud create (UI wiring, create-only)** — new `src/lib/land-records/parties-ui-sync.ts` (`syncPdfIdentitiesToCloud`), called from `src/app/page.tsx`'s save flow right after the geometry sync settles, on branch `sprint-parties-cloud-write` (2026-07-25, **not yet committed/pushed/merged**, pending owner review). Syncs each filled-in `PdfIdentityFields` entry (surveyor/witness/villageHead/applicant -> `surveyor`/`witness`/`village_head`/`original_applicant`; there is no "owner" slot in this form). CREATE-ONLY by owner decision this sprint: `CloudLandParty` has no `updatedAt`, so there is no safe token for `updateCloudParty`'s concurrency filter from the UI layer; a previously-synced identity is re-CREATEd with its persisted id every save instead, relying on the backend's already-verified 23505-retry/duplicate-content check (never a silent overwrite). A client-generated UUID is assigned on first sync and persisted back via a new optional `PdfIdentityPerson.id` field, round-tripped through the existing (unmodified) `STORAGE_KEY` auto-save effect. No UI/layout was changed; sync results are tracked in a new, deliberately unrendered `partiesCloudSync` state. Dev-gated only, same as parent/geometry sync; disabled in Production.
