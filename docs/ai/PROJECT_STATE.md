...
_Last updated: 2026-07-26 (Points cloud-write UI wiring implemented and QA-verified on `sprint-points-cloud-ui-wiring`, based on `main@da79bb0` — **not yet committed, merged, or on `main`**; recorded here ahead of merge per owner instruction so the exact scope is captured while fresh). Previous update: 2026-07-25 (post-merge reconciliation for PR #31, backfilling PR #25–#30 which this file had missed). Update this file at the end of every merged sprint — it is the single source of truth for "what is actually true right now," not the sprint reports themselves._
...
| Dimension | Status |
|---|---|
| Cloud writes | `land_records` parent create/update and the boundary **geometry** sync are merged and wired to the local-first save flow in Dev (PR #24). **Parties** backend create+update (minus `id_number`, ADR-014) and **create-only UI wiring** (four `PdfIdentityFields` entries, via `parties-ui-sync.ts`) are merged via **PR #31** (`main@f98445c8493f1764188ab6de8cf5f905274b0f51`). **Points** create-only backend (PR #21) is now also wired to the UI on `sprint-points-cloud-ui-wiring` (`main@da79bb0` base) — **implemented and QA-verified, not yet merged**. Scope is precise and narrower than "field GPS points" as a whole: only `FieldGpsLite.tsx`'s `points` state (the "Save Point"/Mark Point list — phone-GPS best-fix/averaged capture, keyed coordinate entry, single-point CSV/GeoJSON import) is synced, reported up via a new `onPointsChange` callback and synced via `syncFieldGpsPointsToCloud` (`points-ui-sync.ts`) right after geometry settles. `foundPoints`/`foundPointRecords` (Find Point/AR Guide verification records), `trackLog` (tracked movement), and the single `targetPoint` are explicitly **not** synced by this sprint — do not read "Points cloud-write UI wiring" as full field-GPS coverage. |
| Production UI cloud activation | **Disabled.** Parent `land_records` sync, boundary geometry sync, parties create-only sync, and (pending merge) points create-only sync are wired only behind the non-production, exact-`sabahlot-dev` gate; none are reachable in Production. |
...
- **Points cloud create-only** — backend merged in PR #21 (`main@d071d73`), QA-passed, **still not wired to the UI on `main`**. UI wiring is implemented and QA-verified on `sprint-points-cloud-ui-wiring` (base `main@da79bb0`, 2026-07-26) — new `points-ui-sync.ts` (`syncFieldGpsPointsToCloud`) reports `FieldGpsLite.tsx`'s `points` state (the "Save Point"/Mark Point list only — not `foundPoints`/`foundPointRecords`, `trackLog`, or `targetPoint`) up via a new `onPointsChange` callback, and is called from `page.tsx`'s save flow right after geometry settles and before parties (parent -> geometry -> points -> parties), with a fixed `pointType: "boundary_mark"` for every entry. **Not committed, pushed, or merged — this module stays categorized as "Partial," not "Active in Dev," until that happens.** Full detail is recorded in `docs/ai/MODULE_STATUS.md`'s dated Notes entry (2026-07-26); the live Modules/status-summary tables (this section and `MODULE_STATUS.md`'s table) are deliberately left describing `main`'s actual current, unmerged state and will only be updated in a separate post-merge reconciliation pass, same pattern as PR #31. PR #26 ("complete points UI wiring") remains a distinct, already-merged deliverable — FieldGpsLite's local point list, delete confirmation, rename, and single-point CSV/GeoJSON import bridge — and still never touches `src/lib/land-records/points-*.ts` or calls `createCloudPoint`; it did not close this item. Update/delete remain deliberately deferred because `land_points` has no `updated_at` concurrency token (ADR-011/ADR-013).
...
No new implementation sprint is currently approved to start. Reviewed item-by-item against `main@37f4065` on 2026-07-25 (item 1 updated 2026-07-26 to reflect implementation completed since, on a branch, not yet merged); exactly two substantive items remain open (verified from fact, not forced to a round number) — one implemented and awaiting an owner merge decision, one not yet started at all:

1. **Points cloud-write UI wiring — still open, pending merge.** PR #21 (`main@d071d73`) provides create-only `land_points` backend persistence; as of this writing it is still not wired to the UI on `main`, so this item remains open per this file's own definition of "done" (merged to `main`). **Implementation is complete and QA-verified** on `sprint-points-cloud-ui-wiring` (base `main@da79bb0`, 2026-07-26) — see `docs/ai/MODULE_STATUS.md`'s dated Notes entry for full detail — but it has not been committed, pushed, or merged, so it is not yet reflected in the live Modules/status-summary tables. **PR #26 is not this and still isn't** — PR #26 ("complete points UI wiring") only completed local point-list management UX (FieldGpsLite point list, delete confirmation, inline rename, single-point CSV/GeoJSON import bridge); it never touched `src/lib/land-records/points-*.ts` or called the cloud-write coordinator. Remaining action: owner review and a commit/push/merge decision; once merged, a separate post-merge reconciliation pass (same pattern as PR #31) will update the live tables. Note precisely what is still *not* covered even once this merges: `foundPoints`/`foundPointRecords` (Find Point/AR Guide), `trackLog`, and `targetPoint` remain entirely local-only — a future sprint would need its own explicit scope for any of those.
