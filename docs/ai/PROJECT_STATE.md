...
| `fix/mobile-map-icon-drawer-overlap` | Iterative CSS-only fix series (9 commits through `e133f86`) for mobile topbar/drawer/GPS-control overlap, culminating in a two-tier mobile field header in `src/app/globals.css`. Confirmed 2026-07-23 as the chosen base going forward (owner decision) over the parallel `codex/mobile-field-shell` approach below. Real-device QA confirmation for the accumulated changes is still outstanding — see `sprint-mobile-01-header-drawer-qa-and-cleanup` | Active; not yet merged, real-device QA pending |
| `codex/mobile-field-shell` | Parallel, independently-developed attempt at the same mobile header/drawer overlap problem (Codex agent, separate worktree). Uses a materially different, more invasive approach: a new dedicated `src/app/mobile-field-shell.css` (608 lines) plus structural edits to `src/app/components/Map.tsx` (77 lines) and `src/app/layout.tsx`, versus `fix/mobile-map-icon-drawer-overlap`'s incremental patches to the existing `globals.css` only. 2026-07-23 owner decision: `fix/mobile-map-icon-drawer-overlap` is the chosen base; this branch is **parked/reference-only** — nothing from it is to be merged or cherry-picked without a separate explicit decision | **Parked — preserved unchanged for reference only. Do not push, merge, or delete.** |
