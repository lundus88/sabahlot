...
| Dimension | Status |
|---|---|
| Cloud writes | `land_records` parent create/update and the boundary **geometry** sync are merged and wired to the local-first save flow in Dev (PR #24). Points create-only persistence is merged but not wired to the UI. **Parties** create+update (minus `id_number`, ADR-014) is implemented and QA-passed on the pending `sprint-parties-cloud-write` branch (based on `main@9abe090`, 2026-07-25) — not yet committed/pushed/merged, and not wired to any UI. |
...
- **Parties cloud create+update** — `src/lib/land-records/parties-repository.ts` / `parties-write-coordinator.ts` / `parties-validation.ts` / `parties-cache.ts` (new). Implemented and QA-passed (`parties-write.qa.ts`, 30/30) on branch `sprint-parties-cloud-write` (based on `main@9abe090`, 2026-07-25) — **not yet committed/pushed/merged**, pending owner review. `id_number` (ADR-014) has no field in the writable-input type at all, so it structurally cannot reach any outbound Supabase payload; explicitly regression-tested. Not wired to any UI. `index.ts` does not yet barrel-export this module (out of this sprint's Allowed Files — a future Foundation/Integration sprint should add that additively).
...
| `sprint-parties-cloud-write` | `land_parties` cloud create+update backend (`parties-repository.ts`, `parties-write-coordinator.ts`, `parties-validation.ts`, `parties-cache.ts`, `parties-write.qa.ts`), based on `main@9abe090`. QA-passed (30/30); zero regressions across all 9 pre-existing `.qa.ts` suites (152/152). Not wired to any UI; `index.ts` not updated (out of Allowed Files) | **Pending — not committed/pushed/merged. Awaiting owner review of the sprint report before any commit/push/PR action.** |
