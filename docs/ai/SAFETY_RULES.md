...
- For mobile/CSS layout changes specifically: a simulated-viewport check in a sandboxed browser preview is not equivalent to real-device confirmation. This project's sandbox preview tab has repeatedly rendered as backgrounded/hidden, which silently breaks screenshots, `requestAnimationFrame`, and CSS transform recomputation while `getBoundingClientRect()` geometry checks still work — real bugs have shipped past this exact gap before. State plainly which kind of verification was actually performed; never let a DOM/geometry check read as if it were a real-device visual confirmation.
