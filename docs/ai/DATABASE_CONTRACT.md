...
- **Current implementation status:** Backend (create + update, minus `id_number` per ADR-014) merged via **PR #31** (`main@f98445c8493f1764188ab6de8cf5f905274b0f51`, 2026-07-25). Write: `parties-repository.ts`, `parties-write-coordinator.ts`, `parties-validation.ts`, `parties-cache.ts`. **Create-only** UI wiring is also merged (`parties-ui-sync.ts`, called from `src/app/page.tsx`'s save flow for the four `PdfIdentityFields` entries) — the backend's UPDATE path is not called from the UI yet, since the cached `CloudLandParty` domain type has no `updatedAt` for a safe concurrency check. Dev-gated, disabled in Production. `index.ts` (shared barrel, outside both parties sprints' Allowed Files) still does not re-export this module — a future Foundation/Integration sprint would need to do that additively before other code can consume it via `@/lib/land-records`.
